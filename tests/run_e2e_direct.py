...
# Load webapp_new by file path to avoid import issues caused by the workspace
# path and spaces in the folder name.
...
IMG_PATH = Path('data/temp/2fd455e0-f9db-419e-95b4-922e5869bc02.jpg')
...
cur = conn.cursor()
try:
    username = 'direct_e2e_' + uuid.uuid4().hex[:8]
...
try:
    webapp_new.insert_embedding(emb_cur, user_id, enc, 'direct')
    emb_conn.commit()
...
print('Direct E2E completed. Note: face login requires pgvector and DB function to work.')
