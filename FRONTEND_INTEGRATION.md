# Frontend Integration (Supabase uploads ΓåÆ Backend API)
...
- Create two folders (prefixes) in the Supabase storage bucket:
  - `registration/` ΓÇö images uploaded during user signup/registration
  - `login/` ΓÇö images uploaded to attempt a face login
...
- For `GET/PUT/DELETE /api/users/<user_id>` and image deletion, the server expects the caller identity via header `X-User-Id` or a request field `actor_user_id`. In production, replace this with real authentication (JWT / API key).
...
- The backend will not write to the database during `detect_face` ΓÇö detection is read-only.
